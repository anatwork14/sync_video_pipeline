...
from app.services.feature_based_approach.utils import load_video, get_total_frames, resize_frame
...
def detect_features(frame):
    # Detect features on the entire frame for initialization
    h, w = frame.shape[:2]
    return extract_features_from_frame(frame, (0, 0), (h, w))
...
def construct_trajectories(matches, keypoints1, keypoints2, existing_trajectories=None, match_map=None, threshold=5):
...
def calculate_residual_error(trajectory1, trajectory2):
    ...
    # Standard RANSAC fundamental matrix needs at least 8 point pairs
    if len(trajectory1_unique) < 8:
        return float('inf')

    # Ensure all values are finite and of type float32
    if not np.all(np.isfinite(trajectory1_unique)) or not np.all(np.isfinite(trajectory2_unique)):
        return float('inf')

    try:
        # Perform computation silently. Assertion errors are caught and return inf.
        F, _ = cv2.findFundamentalMat(
            trajectory1_unique.astype(np.float32), 
            trajectory2_unique.astype(np.float32), 
            cv2.FM_RANSAC, 1.0, 0.99, 5000
        )
    except Exception:
        return float('inf')
...
def check_overlap(trajectory1, trajectory2):
    # Trim trajectories to the length of the shorter one
...
def match_trajectories(trajectories1, trajectories2, check_overlap_flag=True):
...
def synchronize_videos(matched_trajectories):
    ...
    # Check for and handle infinite values in y
    valid_indices = np.isfinite(y)  # Get indices of finite values

    # Filter both X and y using valid_indices
    X = X[valid_indices, :]
    y = y[valid_indices]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Multi-Camera Synchronization via Trajectory Matching")
    parser.add_argument('--base_dir', type=str, 
                        default="",
                        help="Directory containing the mp4 files for a single scene")
    parser.add_argument('--no_plots', action='store_true', 
                        help="Disable saving trajectory animations and match plots to speed up processing")
    args = parser.parse_args()
    
    BASE_DIR = args.base_dir
    # Visualization Toggle
    SAVE_PLOT_VIDEO = not args.no_plots
    
    # Find all mp4 files in the BASE_DIR
    import glob
    CAPTURE_FILES = sorted(glob.glob(os.path.join(BASE_DIR, "*.mp4")))
    if not CAPTURE_FILES:
        print(f"Error: No .mp4 files found in {BASE_DIR}")
        return

    print(f"Found {len(CAPTURE_FILES)} cameras: {[os.path.basename(f) for f in CAPTURE_FILES]}")
    total_frames = [get_total_frames(path) for path in CAPTURE_FILES]

    # Organize output folder by scene name
    # BASE_DIR is .../scene1611/videos
    scene_name = os.path.basename(os.path.dirname(BASE_DIR))
    output_dir = os.path.join(os.getcwd(), "results", scene_name)
    ...
    # Store keypoints and descriptors for the first frame of each video
    # We will try a few frames to find the best match for the fundamental matrix
    first_frames = []
    first_frames_keypoints = []
    first_frames_descriptors = []
    trajectories_data = {}
    
    # Search multiple frames for the best initial matching
    SEARCH_FRAMES = 30

    for i, (video, frames) in enumerate(zip(videos, total_frames)):
        # Define camera name and ROI
        cam_name = os.path.splitext(os.path.basename(CAPTURE_FILES[i]))[0]
        
        cap_for_metadata = cv2.VideoCapture(CAPTURE_FILES[i])
        ...
        # Capture frames and find the best one for matching
        best_frame = None
        best_kp = None
        best_desc = None
        max_kp = -1

        # Re-open capture to scan for best frame
        temp_cap = cv2.VideoCapture(CAPTURE_FILES[i])
        ...
        if best_frame is None:
            # Fallback to first frame if detection fails
            temp_cap = cv2.VideoCapture(CAPTURE_FILES[i])
        ...
        # Print the number of features in the best frame
        print(f"Number of features in best frame of video {i}: {len(best_kp) if best_kp else 0}")

        # Initialize VideoWriter if SAVE_PLOT_VIDEO is True
        video_writer = None
        if SAVE_PLOT_VIDEO:
            video_output_path = os.path.join(animation_dir, f"{cam_name}_trajectories.mp4")
            fourcc = cv2.VideoWriter_fourcc(*'mp4v') # Codec for mp4
            # Use original dimensions
            video_writer = cv2.VideoWriter(video_output_path, fourcc, 20.0, (width, height))
            print(f"Started recording trajectory visualization for {cam_name}")

        # Initialize a dictionary to hold the trajectories for this video
        trajectories = {}
        match_map = {} 

        # We need p0 and desc0 for the trajectory matching logic inside the frame loop
        # We use the features from the first frame of the video to start trajectories
        p0 = best_kp
        desc0 = best_desc
        ...
        for frame in tqdm(video, total=frames):
            ...
            # --------------Visualize and Record the trajectories---------
            if SAVE_PLOT_VIDEO and video_writer:
                frame_copy = frame.copy()
                # Draw all trajectories built so far
                for traj in trajectories.values():
                    viz_trajectory = [(int(x), int(y)) for x, y in traj]
                    if len(viz_trajectory) > 1:
                        # Draw with a slight fade or consistent color
                        cv2.polylines(frame_copy, [np.array(viz_trajectory)], False, (0, 255, 0), 2)
                
                # Write the frame with trajectories to the video file
                video_writer.write(frame_copy)

        # Finalize VideoWriter if active
        if video_writer:
            video_writer.release()
            print(f"Finished recording trajectory visualization for {cam_name}")
        ...
        if i > 0:
            ...
            # Save the image with matches to the scene-specific output directory
            match_img_path = os.path.join(output_dir, f'matches_{cam_name}.png')
            cv2.imwrite(match_img_path, img_matches)
            print(f"Saved match visualization to {match_img_path}")
        ...
        # for traj in filtered_trajectories[:5]:  # Print the first 5 trajectories
        #     print(traj)

        # print("Lengths of Example Trajectories:")
        # for traj in filtered_trajectories[:5]:  # Print the lengths of the first 5 trajectories
        #     print(len(traj))


        # Visualize the filtered trajectories on the current video ToDO
        # visualize_trajectories(CAPTURE_FILES[i], filtered_trajectories)
        ...
        # Add the filtered trajectories for this video to the trajectories data
    ...
    ref_name = os.path.splitext(os.path.basename(CAPTURE_FILES[0]))[0]
    if len(trajectories_data) > 1:
        # Try to find at least one other successful camera to compute F
        other_cams = [name for name in trajectories_data.keys() if name != ref_name]
        if other_cams:
            target_cam = other_cams[0]
            # Find index of target_cam
            target_idx = [j for j, f in enumerate(CAPTURE_FILES) if os.path.basename(f).startswith(target_cam)][0]
            ...
            if F is not None and F.shape == (3, 3):
                print(f'Performing additional filtering on reference camera {ref_name} using {target_cam}')
                trajectories_data[ref_name] = filter_trajectories(trajectories_data[ref_name], F)
                print(f"Number of filtered trajectories in reference video: {len(trajectories_data[ref_name])}")
    ...
    for i in range(1, len(CAPTURE_FILES)):
        cam_name = os.path.splitext(os.path.basename(CAPTURE_FILES[i]))[0]
        if cam_name not in trajectories_data or not trajectories_data[cam_name]:
            print(f"Skipping {cam_name} due to missing trajectories.")
            continue
        ...
        print(f"Number of matched trajectories between {ref_name} and {cam_name}: {len(matched_trajectories)}")

        # Synchronize the videos
        if not matched_trajectories:
            print(f"Number of matched trajectories between {ref_name} and {cam_name}: 0. Skipping sync for this camera.")
            continue
        ...
        sync_dict[cam_name] = int(adjusted_offsets[1])

    # Add some runtime metrics to the results
    metrics = {
        'total_cameras_found': len(CAPTURE_FILES),
        'cameras_successfully_synced': len(sync_dict),
        'average_matched_trajectories': 0 if len(sync_dict) <= 1 else sum([len(t) for t in trajectories_data.values()]) / max(1, len(trajectories_data))
    }
    
    # Print and save the synchronization parameters
    results = {'sync': sync_dict, 'metrics': metrics}
    print(results)
    
    import json
    json_path = os.path.join(output_dir, 'sync_results.json')
